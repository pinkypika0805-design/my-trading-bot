...
st.set_page_config(page_title="Annex Garage 交易系統 V5.2", page_icon="🏎️")
st.title("🏹 精準當沖進場檢核 (V5.2)")
st.caption("實驗目標：每日一單 (中文名稱同步版)，嚴格執行 09:10 紀律")
...
if st.sidebar.button("🔍 自動抓取今日數據"):
    try:
        # 台灣股市代號處理
        t_code = f"{ticker_input}.TW"
        stock = yf.Ticker(t_code)
        hist = stock.history(period="2d")
        
        if hist.empty:
            t_code = f"{ticker_input}.TWO"
            stock = yf.Ticker(t_code)
            hist = stock.history(period="2d")
        ...
        if not hist.empty:
            # 抓取中文名稱
            info = stock.info
            # 優先找 longName (通常包含中文)，若無則找 shortName
            c_name = info.get('longName') or info.get('shortName') or ticker_input
            # 簡單過濾掉常見的英文後綴，盡量呈現中文
            c_name = c_name.replace("Taiwan Semiconductor Manufacturing Company Limited", "台積電")
            
            st.session_state.auto_data["name"] = c_name
            ...
            st.sidebar.success(f"✅ 已抓取：{c_name}")
        else:
            st.sidebar.error("找不到該股號")
    except:
        st.sidebar.error("抓取失敗，請確認網路")

# 左側同步顯示名稱
st.sidebar.markdown(f"**當前標的：{st.session_state.auto_data['name']}**")
...
with col3:
    ...
    exhaustion_signal = st.checkbox("🚩 高點力竭" if trade_type == "做多 (Long)" else "🎯 底部力竭")
with col4:
